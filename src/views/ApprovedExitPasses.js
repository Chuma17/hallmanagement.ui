...
const ApprovedExitPasses = () => {
    ...
    useEffect(() => {
        async function getExitPasses() {
            ...
            try {
                const { data } = await axios.get(`https://localhost:44324/api/ExitPass/get-approved-exitPasses`,
                    {
                        headers: {
    ...
    return <>
        <div className="container ms-auto me-auto mt-3">
            ...
            <div style={{ borderRadius: "10px" }} className="d-flex justify-content-between bg-dark p-4 text-light">
                ...
                <h3>APPROVED EXIT PASSES [ {exitPasses.length} ]</h3>
                <Link to="/exit-passes"><button className="btn btn-success">All Passes</button></Link>
            </div>
...
export default ApprovedExitPasses;