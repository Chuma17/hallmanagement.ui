...
const ComplaintsInHall = () => {    
    ...
    useEffect(() => {
        async function getComplaints() {
            ...
            try {
                const { data } = await axios.get(`https://localhost:44324/api/ComplaintForm/get-complaints-in-hall/${hallId}`,
                    {
                        headers: {
    ...
    return <>
        ...
        <div className="container ms-auto me-auto mt-3">
            ...
            <table className="table table-striped mt-4 fs-5">
                ...
                <tbody>
                    {
                        complaints.length > 0 ? complaints.map((user, i) => (
                            <tr>
                                <th scope="row">{i + 1}</th>
                                <td>{user?.plumbing}</td>
...
export default ComplaintsInHall;